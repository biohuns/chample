...
short parity_move[2][18] = {{1, 0, 1, 1, 0, 1, 1, 0, 1, 1, 0, 1, 1, 0, 1, 1, 0, 1},
...
// Move table for permutation of six corners. The positions of the DBL and DRB
// corners are determined by the parity. URFtoDLF < 20160 in phase 1 URFtoDLF <
// 20160 in phase 2 URFtoDLF = 0 for solved cube.
short URFtoDLF_move[N_URFtoDLF][N_MOVE];
// Move table for the permutation of six U-face and D-face edges in phase2. The
// positions of the DL and DB edges are determined by the parity. URtoDF <
// 665280 in phase 1 URtoDF < 20160 in phase 2 URtoDF = 0 for solved cube.
          ...
          for (j = 0; j < 18; j++) {
            switch (j) {
              ...
              default:
                ...
                if (get_pruning(slice_URFtoDLF_parity_prun, (N_SLICE2 * new_URFtoDLF + new_slice) * 2 + new_parity) ==
                    0x0f) {
                  set_pruning(slice_URFtoDLF_parity_prun, (N_SLICE2 * new_URFtoDLF + new_slice) * 2 + new_parity,
                              depth + 1);
                  done++;
                }
            }
          ...
          for (j = 0; j < 18; j++) {
            switch (j) {
              ...
              default:
                ...
                new_parity = parity_move[parity][j];
                if (get_pruning(slice_URtoDF_parity_prun, (N_SLICE2 * new_URtoDF + new_slice) * 2 + new_parity) ==
                    0x0f) {
                  set_pruning(slice_URtoDF_parity_prun, (N_SLICE2 * new_URtoDF + new_slice) * 2 + new_parity,
                              depth + 1);
                  done++;
                }
            }
