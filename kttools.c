...
// 0以上(max-1)の乱数を返す
int next_int(int max) { return (int)(((double)(genrand_int32() & 0x7fffffff) / (unsigned long)0x80000000) * max); }
 ...
 * @return A random cube with parity in the string representation. Each cube of
 * the cube space has the same probability.
 */
 ...
 * @return A random cube with no parity in the string representation. Each cube
 * of the cube space has the same probability.
 */
 ...
 * @return A edge-randomized cube in the string representation. Each cube of the
 * cube space has the same probability.
 */
 ...
 * @return A corner-randomized cube in the string representation. Each cube of
 * the cube space has the same probability.
 */
...
void yy_cube_test1(char *des) {
  ...
  do {
    ...
    set_twist(&cc, next_int(N_TWIST));
    set_URFtoDLB(&cc, next_int(N_URFtoDLB));
  } while ((edge_parity(&cc) ^ corner_parity(&cc)) != 0 /*|| corner_roop(&cc) != 2*/
           || count_co(&cc) != 4);
  to_facecube(&cc, &fc);
  to_string(&fc, des);
}
...
void yy_cube_test2(char *des) {
  ...
  } while ((edge_parity(&cc) ^ corner_parity(&cc)) != 0 || parity != 1 || corner_roop(&cc) != 1 ||
           edge_number(&cc, parity) != 14 || count_eo(&cc, parity) != 0 || count_co(&cc) != 0);
  to_facecube(&cc, &fc);
  to_string(&fc, des);
}
